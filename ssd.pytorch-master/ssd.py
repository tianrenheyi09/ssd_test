...
def my_vgg(i, batch_norm=False):
    in_channels = i
    layers = []
    layers += [nn.Conv2d(3,64,kernel_size=(3,3),stride=(1,1),padding=(1,1))]
    ...
    #######
    layers += [nn.Conv2d(512,512,kernel_size=3,stride=3,padding=1)]
    layers += [nn.ReLU(inplace=True)]
    layers += [nn.Conv2d(512,512,kernel_size=3,stride=1,padding=1)]
    layers += [nn.ReLU(inplace=True)]
    layers += [nn.Conv2d(512,512,kernel_size=3,stride=1,padding=1)]
    layers += [nn.ReLU(inplace=True)]
    layers += [nn.MaxPool2d(kernel_size=3,stride=1,padding=1,dilation=1)]
    ######
    layers += [nn.Conv2d(512,1024,kernel_size=3,stride=1,padding=6,dilation=6)]
    layers += [nn.ReLU(inplace=True)]
...
def my_extra_layers(i, batch_norm=False):
    in_channels = 3
    layers = []
...
extra_layer = add_extras(extras[str(size)], 1024),
...
base_, extras_, head_ = multibox(my_vgg(3),
                                     my_extra_layers(3),
                                     mbox[str(size)], num_classes)
#
#
...
#
out = SSD('train', 300, base_, extras_, head_, num_classes)
out = build_ssd('train')
y = out(x)
#
# import cv2
...
#     cv2.rectangle(img,(b[i][0],b[i][1]),(b[i][2],b[i][3]),(0,255,0),3)
# # cv2.rectangle(img,(b[0][0],b[0][1]),(b[0][2],b[0][3]),(0,255,0),3)
